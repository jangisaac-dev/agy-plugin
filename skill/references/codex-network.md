# Codex sandbox / network notes for agy-bridge

The `agy` CLI calls a remote backend (network) and writes its own config/session
files outside the workspace (~/.gemini/antigravity-cli). Codex's default sandbox blocks both, so runs
...
The agy-bridge runner only ever modifies an isolated git worktree (or nothing, for
read-only commands) — it does not target your live working tree. Residual risk:
`agy`, running outside the sandbox, could in principle write to absolute paths
