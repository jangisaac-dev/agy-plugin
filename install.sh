#!/usr/bin/env bash
# install.sh — register the agy bridge as a local marketplace and enable it.
#
...
case "$MODE" in
  ...
  --apply)
    ...
    echo
    echo "Done. Start a new Claude Code session and try /agy:review."
    ;;
  *)
